...
namespace IBHFL.Models
{
    public class UtilityModel
    {
        ...
        {   
            ...
            command.CommandText = str;
            MySqlDataReader mySqlDataReader = command.ExecuteReader();
            string categoryName = ""; // Define variable outside the loop
            while (mySqlDataReader.Read())
            {
                categoryName = mySqlDataReader["categoryname"].ToString(); // Assign value inside the loop
            }
            this.conn.Close();
            return categoryName;
        }
        ...
        {
            MySqlCommand command = this.conn.CreateCommand();
            string str = "INSERT INTO tbl_certificate_log(id_assessment, id_user, score, certificateFileName, addedDate, attempt_no, pdfURL) VALUES (@value1,@value2,@value3, @value4,@value5,@value6,@value7)";
            command.CommandText = str;
